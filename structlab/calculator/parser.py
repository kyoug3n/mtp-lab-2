...
ЧИСЛО — целое, дробное или мнимое (``4i``, ``i``). Каждому правилу
соответствует метод разборщика. Правила ссылаются друг на друга, отсюда
рекурсия: выражение в скобках разбирается тем же кодом, что и всё
выражение. Из грамматики следует:
...
* ``sqrt`` применяется к ближайшему числу или скобке, как вызов функции:
  ``sqrt 16 + 9 = 13``, ``sqrt(16 + 9) = 5``, ``sqrt(-4) ^ 2 = -4``.
...
class _Parser:
    ...
    def _primary(self) -> Number:
        ...
        if token.kind == FUNCTION:
            return square_root(self._primary(), token.position)
